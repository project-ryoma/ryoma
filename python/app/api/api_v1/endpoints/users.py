...
from app.models import Message, User, UserCreate, UserCreateOpen, UserOut, UserUpdate, UserUpdateMe
...
def update_user_me(*, session: SessionDep, body: UserUpdateMe, current_user: CurrentUser) -> Any:
    """
    Update own user.
    """
...
def delete_user(session: SessionDep, current_user: CurrentUser, user_id: int) -> Message:
    ...
    if user == current_user:
        raise HTTPException(status_code=400, detail="Users are not allowed to delete themselves")
    session.delete(user)
    session.commit()
