...
class IcebergConfig(BaseModel):
    ...
    properties: Optional[Dict[str, str]] = Field(
        None, description="Additional catalog properties"
    )
...
class IcebergDataSource(DataSource):
    ...
    def _get_catalog(self) -> IcebergCatalog:
        ...
        if self._catalog is None:
            ...
            if self.catalog_type == "rest":
                ...
                catalog_props.update({"uri": self.catalog_uri, "type": "rest"})
                if self.warehouse:
                ...
                catalog_props.update({"type": "hive"})
                if self.catalog_uri:
                ...
                catalog_props.update({"type": "glue"})
                ...
                try:
                    ...
                    for table_id in table_identifiers:
                        ...
                        enhanced_tables.append(
                            self._build_enhanced_table(schema, table_name)
                        )
                    ...
                    self.logger.warning(
                        f"Could not list tables in namespace {schema}: {e}"
                    )
                    ...
                    for namespace in namespaces:
                        ...
                        try:
                            ...
                            for table_id in table_identifiers:
                                ...
                                enhanced_tables.append(
                                    self._build_enhanced_table(schema_name, table_name)
                                )

                            enhanced_schemas.append(
                                Schema(schema_name=schema_name, tables=enhanced_tables)
                            )
                        except Exception as e:
                            self.logger.warning(
                                f"Could not process namespace {namespace}: {e}"
                            )
                            continue

                    return Catalog(
                        catalog_name=self.catalog_name, schemas=enhanced_schemas
                    )
                except Exception as e:
    ...
    def _build_enhanced_table(self, namespace: str, table_name: str) -> Table:
        ...
        try:
            ...
            for field in schema.fields:
                column_profile = self._build_column_profile_from_iceberg(
                    field, iceberg_table
                )
                ...
                column = Column(
                    ...
                    in self._get_identity_fields(iceberg_table),
                    profile=column_profile,
                )
                columns.append(column)

            # Build table profile
            table_profile = self._build_table_profile_from_iceberg(
                table_name, iceberg_table
            )

            return Table(table_name=table_name, columns=columns, profile=table_profile)
    ...
    def _build_column_profile_from_iceberg(
        ...
        try:
            ...
            if latest_snapshot and latest_snapshot.summary:
                ...
                row_count = (
                    int(summary.get("total-records", 0))
                ...
                if "total-data-files" in summary:
                    ...
                    if row_count:
                        ...
                        distinct_count = min(
                            row_count, row_count // max(1, file_count // 10)
                        )
            ...
            if any(
                t in field_type_str
                for t in ["int", "long", "float", "double", "decimal"]
            ):
                numeric_stats = NumericStats(min_value=min_value, max_value=max_value)
            elif any(t in field_type_str for t in ["date", "timestamp"]):
                if min_value and max_value:
                    try:
                        date_stats = DateStats(
                            min_date=(
                                min_value if isinstance(min_value, datetime) else None
                            ),
                            max_date=(
                                max_value if isinstance(max_value, datetime) else None
                            ),
                        )
                    except Exception:
                        pass
            ...
            self.logger.error(
                f"Error building column profile for {field.name}: {str(e)}"
            )
            return ColumnProfile(column_name=field.name, profiled_at=datetime.now())

    def _build_table_profile_from_iceberg(
        self, table_name: str, iceberg_table: Any
    ) -> TableProfile:
        ...
        try:
            ...
            if latest_snapshot:
                if latest_snapshot.summary:
                    ...
                    table_size_bytes = (
                        int(summary.get("total-data-files-size", 0))
                        if "total-data-files-size" in summary
                        else None
                    )

                # Get timestamp of latest snapshot
                last_updated = datetime.fromtimestamp(
                    latest_snapshot.timestamp_ms / 1000
                )
            ...
            self.logger.error(
                f"Error building table profile for {table_name}: {str(e)}"
            )
            return TableProfile(table_name=table_name, profiled_at=datetime.now())
    ...
    def _calculate_quality_score(
        self, null_percentage: float, distinct_ratio: float
    ) -> float:
        """Calculate data quality score."""
    ...
    def crawl_catalog(self, loader, **kwargs):
        ...
        try:
            ...
            self.logger.info(
                f"Crawled {len(catalog.schemas or [])} namespaces from Iceberg catalog"
            )
            return catalog
        except Exception as e:
