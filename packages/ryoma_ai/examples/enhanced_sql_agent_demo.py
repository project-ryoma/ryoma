...
def demo_schema_linking():
    ...
    for question in questions:
        ...
        try:
            ...
            print(f"Relevant tables: {[t['table'] for t in analysis['relevant_tables'][:3]]}")
            print(f"Confidence: {analysis['confidence_score']}")
            
            # Show table selection
            table_suggestions = schema_agent.suggest_table_selection(question, max_tables=3)
...
def demo_error_handling():
    ...
    error_scenarios = [
        ("SELECT * FROM non_existent_table", "Table 'non_existent_table' doesn't exist"),
        ...
        ("SELECT * FROM customers ORDER BY", "Syntax error: missing column after ORDER BY")
    ]
    ...
    for query, error_msg in error_scenarios:
        ...
        try:
            ...
            for strategy in strategies[:2]:  # Show top 2 strategies
                print(f"  - {strategy.description} (confidence: {strategy.confidence:.2f})")
...
def demo_advanced_tools():
    ...
    tools = [
        ...
        (QueryValidationTool(), {"query": "SELECT * FROM customers", "datasource": datasource}),
        (TableSelectionTool(), {"question": "Show customer orders", "datasource": datasource}),
        (QueryOptimizationTool(), {"query": "SELECT * FROM customers ORDER BY name", "datasource": datasource}),
        (QueryExplanationTool(), {"query": "SELECT COUNT(*) FROM orders WHERE date > '2023-01-01'", "datasource": datasource})
    ]
...
def demo_configuration_options():
    ...
    configs = [
        ...
        },
        {
            ...
            "config": {
                ...
                "protected_tables": ["information_schema", "mysql", "performance_schema"]
            }
        }
    ]
    ...
    for config_info in configs:
        ...
        validator = SqlSafetyValidator(
            datasource=datasource,
            safety_config=config_info['config']
        )
