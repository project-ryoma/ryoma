...
def demo_comprehensive_profiling():
    ...
    if "error" not in table_profile:
        ...
        for col_name, profile in column_profiles.items():
            ...
            if numeric_stats:
                print(f"      Range: {numeric_stats.get('min_value', 'N/A')} - {numeric_stats.get('max_value', 'N/A')}")
                print(f"      Mean: {numeric_stats.get('mean', 'N/A'):.2f}" if numeric_stats.get('mean') else "")
            
            # Show string stats if available
            string_stats = profile.get('string_stats')
            if string_stats:
                print(f"      Length: {string_stats.get('min_length', 'N/A')} - {string_stats.get('max_length', 'N/A')} chars")
                print(f"      Avg length: {string_stats.get('avg_length', 'N/A'):.1f}" if string_stats.get('avg_length') else "")
            
            # Show date stats if available
            date_stats = profile.get('date_stats')
            if date_stats:
                print(f"      Date range: {date_stats.get('date_range_days', 'N/A')} days")
    else:
    ...
    for schema in enhanced_catalog.schemas or []:
        ...
        for table in schema.tables or []:
            ...
            if table.profile:
                ...
                print(f"         Completeness: {table.profile.completeness_score:.3f}" if table.profile.completeness_score else "")
...
def demo_individual_column_profiling():
    ...
    for column_name in columns_to_profile:
        ...
        if "error" not in column_profile:
            ...
            print(f"   ❌ NULL percentage: {column_profile.get('null_percentage', 0):.1f}%")
            ...
            if column_profile.get('numeric_stats'):
                ...
                print(f"      Std Dev: {stats.get('std_dev', 'N/A'):.2f}" if stats.get('std_dev') else "")
            ...
            if lsh_sketch:
                print(f"   🔗 LSH sketch: {lsh_sketch['num_hashes']} hashes, threshold {lsh_sketch['jaccard_threshold']}")
        else:
