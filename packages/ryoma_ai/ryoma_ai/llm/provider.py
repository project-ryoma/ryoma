...
def _load_chat_model(provider_id: str, model_name: str, model_parameters: Dict) -> Any:
    ...
    logging.info(f"Loading chat model with provider: {provider_id}, model: {model_name}, parameters: {model_parameters}")
    if provider_id in ["openai", "openai-chat"]:
        try:
            from langchain_openai import ChatOpenAI
            return ChatOpenAI(model=model_name, **model_parameters)
        except ImportError:
            logging.error("langchain_openai not available. Install with: pip install langchain-openai")
            return None
        ...
        try:
            ...
            logging.error("GPT4All dependencies not available. Install with: pip install gpt4all")
            return None
        ...
        try:
            ...
            logging.error("langchain_huggingface not available. Install with: pip install langchain-huggingface")
            return None
        ...
        try:
            ...
            logging.error("langchain_anthropic not available. Install with: pip install langchain-anthropic")
            return None
...
def _load_embedding_model(provider_id: str, model_name: str, model_parameters: Dict) -> Optional[Embeddings]:
