...
from ryoma_ai.prompt.core import (
    ExampleFormatter, ExampleSelector, PromptConfig, 
    PromptType, ExampleFormat, SelectorStrategy
)
...
class SQLPromptBuilder:
    ...
    def build(self, config: PromptConfig, context: Dict[str, Any]) -> ChatPromptTemplate:
...
def register_default_components():
    ...
    prompt_registry.register_builder(PromptType.INSTRUCTION_FOLLOWING, InstructionPromptBuilder())
    prompt_registry.register_builder(PromptType.CHAIN_OF_THOUGHT, ChainOfThoughtPromptBuilder())
    
    # Register selectors
    prompt_registry.register_selector(SelectorStrategy.RANDOM, RandomExampleSelector())
    prompt_registry.register_selector(SelectorStrategy.SIMILARITY, SimilarityExampleSelector())
    prompt_registry.register_selector(SelectorStrategy.COSINE, SimilarityExampleSelector())  # alias
    
    # Register formatters
    prompt_registry.register_formatter(ExampleFormat.QUESTION_ANSWER, QuestionAnswerFormatter())
    prompt_registry.register_formatter(ExampleFormat.SQL_ONLY, SQLOnlyFormatter())
    prompt_registry.register_formatter(ExampleFormat.COMPLETE_CONTEXT, CompleteContextFormatter())
