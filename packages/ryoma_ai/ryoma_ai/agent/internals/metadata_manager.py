...
class MetadataManager:
    ...
    async def initialize_batch_metadata(
        ...
        self.logger.info(f"Starting batch metadata initialization (mode: {self.mode.value})")
        ...
        try:
            if tables:
                ...
                if schemas:
                    ...
                    for schema_obj in catalog.schemas or []:
                        for table in schema_obj.tables or []:
                            if self._should_include_table(table.table_name, schema_obj.schema_name):
                                discovered_tables.append((table.table_name, schema_obj.schema_name))
    ...
    def _should_include_table(self, table_name: str, schema: Optional[str]) -> bool:
        ...
        if not self.batch_config.include_system_tables:
            ...
            if any(table_name.lower().startswith(pattern) for pattern in system_patterns):
                return False
            if schema and schema.lower() in ['information_schema', 'pg_catalog', 'mysql', 'sys']:
                return False
    ...
    def _process_table_chunk(
        self, 
        table_chunk: List[Tuple[str, Optional[str]]]
    ) -> Dict[str, Dict[str, Any]]:
        ...
        for table_name, schema in table_chunk:
            try:
                ...
                table_profile = self.profiler.profile_table(self.datasource, table_name, schema)
                    ...
                    for schema_obj in catalog.schemas:
                        ...
                        if table_obj:
                            for column in table_obj.columns:
                                ...
                                enhanced_metadata = self.profiler.generate_field_description(
                                    column_profile, table_name, schema
                                )
                                column_metadata[column.name] = {
                                    'profile': column_profile.model_dump() if hasattr(column_profile, 'model_dump') else column_profile.__dict__,
                                    'enhanced': enhanced_metadata
                                }
                
                chunk_results[table_key] = {
                    'table_profile': table_profile.model_dump() if hasattr(table_profile, 'model_dump') else table_profile.__dict__,
        ...
        try:
            ...
            for table_name, schema in tables:
                ...
                if table_key in self._metadata_cache:
                    ...
                    for column_name, column_data in table_metadata.get('columns', {}).items():
    ...
    async def get_realtime_metadata(
        ...
        """
        self.logger.info(f"Generating real-time metadata for question: {question[:100]}...")
        start_time = time.time()
        ...
        try:
            ...
            metadata_context = MetadataContext(
                ...
                coverage_stats={'mode': 'realtime', 'tables_analyzed': len(relevant_tables)}
            )
            ...
            for schema_obj in catalog.schemas or []:
                for table in schema_obj.tables or []:
                    ...
                    if (table_name in question_lower or 
                        any(word in table_name for word in question_lower.split())):
                        full_name = f"{schema_obj.schema_name}.{table.table_name}" if schema_obj.schema_name else table.table_name
                ...
                for schema_obj in catalog.schemas:
                    ...
                    if table_obj:
                        for column in table_obj.columns:
                            ...
                            enhanced_metadata['question_relevance'] = self._assess_column_relevance(
    ...
    async def get_hybrid_metadata(
        ...
        if not relevant_tables:
            relevant_tables = await self._select_relevant_tables_for_question(question, 10)
    ...
    def _needs_batch_refresh(self) -> bool:
        ...
        hours_since_update = (datetime.now() - self._last_batch_update).total_seconds() / 3600
    ...
    def _parse_table_identifier(self, table_identifier: str) -> Tuple[Optional[str], str]:
    ...
    async def get_metadata_for_question(
        ...
        if self.mode == MetadataMode.BATCH:
            ...
            return await self.get_realtime_metadata(question, relevant_tables, max_tables)
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_batch_update': self._last_batch_update.isoformat() if self._last_batch_update else None,
