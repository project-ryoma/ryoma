...
class SafetyLevel(IntEnum):
    ...
    def name_str(self) -> str:
        ...
        return {1: "safe", 2: "warning", 3: "dangerous", 4: "blocked"}[self.value]
...
class SqlSafetyValidator:
    ...
    def validate_query(
        self, sql_query: str, context: Optional[Dict] = None
    ) -> ValidationResult:
        """
        ...
        execution_allowed = safety_level not in [
        ...
        sanitized_query = (
            self._sanitize_query(sql_query, violations) if violations else None
        )
    ...
    def _apply_rule(
        self, rule: ValidationRule, sql_query: str, context: Optional[Dict]
    ...
    def _sanitize_query(
        self, sql_query: str, violations: List[SafetyViolation]
    ) -> Optional[str]:
    ...
    def create_safety_report(self, validation_result: ValidationResult) -> str:
        ...
        report += (
            f"Overall Safety Level: {validation_result.safety_level.name_str.upper()}\n"
        )
    ...
    def get_safety_recommendations(self, sql_query: str) -> List[str]:
        ...
        recommendations.append(
            "Limit result sets with appropriate WHERE clauses and LIMIT"
        )
        recommendations.append("Use explicit column names instead of SELECT *")
        recommendations.append(
            "Test queries on small datasets before running on production"
        )

        # Query-specific recommendations
        if "JOIN" in query_upper:
            recommendations.append(
                "Ensure proper indexing on join columns for performance"
            )

        if "ORDER BY" in query_upper:
            recommendations.append(
                "Consider adding LIMIT when using ORDER BY to avoid sorting large datasets"
            )

        if re.search(r"\b(DELETE|UPDATE|INSERT)\b", query_upper):
            recommendations.append(
                "Always backup data before running modification queries"
            )
            recommendations.append(
                "Test modification queries with WHERE 1=0 first to check syntax"
            )
