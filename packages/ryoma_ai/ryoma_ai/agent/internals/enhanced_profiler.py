...
class EnhancedDatabaseProfiler:
    ...
    def profile_table_enhanced(
        ...
        join_patterns = self._analyze_join_patterns(
            table_name, field_metadata, schema
        )
    ...
    def profile_field_enhanced(
        ...
        cache_key = f"{schema}.{table_name}.{field_name}" if schema else f"{table_name}.{field_name}"
        ...
        join_score = self._calculate_enhanced_join_score(
            base_profile, llm_analysis
        )
        
        # Step 4: Generate SQL-specific usage patterns
        usage_patterns = self._generate_sql_usage_patterns(
            base_profile, llm_analysis
        )
        ...
        enhanced_metadata = EnhancedFieldMetadata(
            ...
            llm_description=llm_analysis.get('description', f'Field containing {base_profile.column_name} data'),
            ...
            semantic_tags=self._generate_enhanced_semantic_tags(base_profile, llm_analysis),
            data_quality_assessment=llm_analysis.get('data_quality', 'Assessment unavailable'),
    ...
    def _generate_field_analysis(
        ...
        try:
            ...
            logger.error(f"LLM field analysis failed for {profile.column_name}: {str(e)}")
...
"""
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                return {
                    ...
                    'data_quality': parsed.get('data_quality', 'Quality assessment unavailable'),
    ...
    def _create_fallback_analysis(self, profile: ColumnProfile) -> Dict[str, Any]:
        ...
        if profile.semantic_type == 'identifier':
            ...
            hints.append("Field contains nulls - consider using COALESCE or IS NULL checks")
    ...
    def _generate_table_analysis(
        ...
        field_summary = []
        for field_name, metadata in list(field_metadata.items())[:10]:  # Limit for prompt size
        ...
        for field_name, metadata in field_metadata.items():
            if metadata.join_candidate_score > 0.5:  # High join candidate score
                pattern = {
                    ...
                    'type': 'primary_key' if metadata.join_candidate_score > 0.8 else 'foreign_key',
    ...
    def get_sql_generation_context(
        ...
        for field_name, field_meta in enhanced_metadata.field_metadata.items():
            field_contexts[field_name] = {
                ...
                'sample_values': [
                    item.get('value') for item in (field_meta.base_profile.top_k_values or [])[:3]
                ],
