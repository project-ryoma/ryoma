...
class QueryPlannerAgent(ChatAgent):
    ...
    def create_query_plan(self, question: str, context: Optional[Dict] = None) -> QueryPlan:
        """
    ...
    def optimize_query_plan(self, plan: QueryPlan) -> QueryPlan:
        ...
        return QueryPlan(
            ...
            total_estimated_time=sum(step.execution_time_estimate or 0 for step in reordered_steps),
            optimization_notes=plan.optimization_notes + ["Plan optimized for performance"]
        )
    ...
    def _analyze_question_complexity(self, question: str) -> QueryComplexity:
        ...
        entities = len(re.findall(r'\b(?:customer|order|product|employee|sale|payment)\w*\b', question_lower))
    ...
    def _create_simple_plan(self, question: str, context: Optional[Dict]) -> List[QueryStep]:
    ...
    def _create_moderate_plan(self, question: str, context: Optional[Dict]) -> List[QueryStep]:
    ...
    def _create_complex_plan(self, question: str, context: Optional[Dict]) -> List[QueryStep]:
    ...
    def _optimize_query_step(self, step: QueryStep) -> QueryStep:
        ...
        if "ORDER BY" in step.sql_query and "LIMIT" not in step.sql_query:
            optimized_time = (optimized_time or 0) * 1.3  # Penalty for unlimited ORDER BY
    ...
    def _combine_steps_if_possible(self, steps: List[QueryStep]) -> List[QueryStep]:
        ...
        for i, step in enumerate(steps):
            ...
            if (i + 1 < len(steps) and 
                len(step.depends_on) == 0 and 
                steps[i + 1].depends_on == [step.step_id]):
                ...
                combined_step = QueryStep(
                    ...
                    depends_on=step.depends_on,
                    execution_time_estimate=(step.execution_time_estimate or 0) + (steps[i + 1].execution_time_estimate or 0) * 0.8
                )
    ...
    def _generate_optimization_notes(self, steps: List[QueryStep], question: str) -> List[str]:
        ...
        if "aggregate" in question.lower() or "sum" in question.lower():
            notes.append("Consider using materialized views for frequently accessed aggregations")
    ...
    def explain_plan(self, plan: QueryPlan) -> str:
        ...
        for step in plan.steps:
            ...
            if step.depends_on:
                explanation += f"     Depends on: {', '.join(map(str, step.depends_on))}\n"
            explanation += f"     Estimated time: {step.execution_time_estimate:.1f}s\n\n"
    ...
    def estimate_query_cost(self, sql_query: str) -> Dict[str, Any]:
        ...
        cost_factors = {
            ...
            "aggregations": sum(query_upper.count(agg) for agg in ["SUM(", "COUNT(", "AVG(", "MAX(", "MIN("]),
    ...
    def _get_performance_recommendations(self, cost_factors: Dict[str, int]) -> List[str]:
        ...
        if cost_factors["subqueries"] > 2:
            recommendations.append("Consider converting subqueries to joins where possible")
        
        if cost_factors["sorts"] > 1:
            recommendations.append("Multiple ORDER BY clauses detected - consider combining or using indexes")
        
        if cost_factors["aggregations"] > 3:
            recommendations.append("Heavy aggregation detected - consider using summary tables")
