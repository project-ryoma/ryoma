...
class CatalogNotFoundError(StoreException):
    ...
    def __init__(self, catalog_id: str, data_source_id: Optional[str] = None, cause: Optional[Exception] = None):
        if data_source_id:
            message = f"Catalog '{catalog_id}' not found for data source '{data_source_id}'"
        else:
...
class CatalogIndexError(StoreException):
    ...
    def __init__(self, operation: str, catalog_id: str, cause: Optional[Exception] = None):
...
class DataSourceConnectionError(StoreException):
    ...
    def __init__(self, data_source_id: str, connection_error: str, cause: Optional[Exception] = None):
        message = f"Failed to connect to data source '{data_source_id}': {connection_error}"
        super().__init__(message, cause)
        self.data_source_id = data_source_id
