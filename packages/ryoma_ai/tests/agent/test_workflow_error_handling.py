...
class TestWorkflowAgentErrorHandling:
    ...
    def test_model_initialization_failure_error_message(self):
        ...
        with patch("ryoma_ai.llm.provider.load_model_provider", side_effect=Exception("Model not found")):
    ...
    def test_chain_build_with_failed_model_initialization(self):
        ...
        with patch("ryoma_ai.llm.provider.load_model_provider") as mock_load:
            ...
            with pytest.raises(ValueError, match="Unable to initialize model 'gemini:gemini-2.0-flash'"):
                agent._build_chain()
    ...
    def test_tools_binding_fallback(self):
        ...
        with patch("ryoma_ai.llm.provider.load_model_provider") as mock_load:
            ...
            with patch("ryoma_ai.agent.workflow.render_text_description") as mock_render:
    ...
    def test_chain_building_with_prompt_template(self):
        ...
        with patch("ryoma_ai.llm.provider.load_model_provider") as mock_load:
            ...
            with patch("ryoma_ai.prompt.prompt_template.PromptTemplateFactory") as mock_factory_class:
                mock_factory = Mock()
                mock_template = Mock()
...
class TestWorkflowAgentIntegration:
    ...
    def test_error_handling_with_missing_dependencies(self):
        ...
        with patch("ryoma_ai.llm.provider.load_model_provider", side_effect=ImportError("Failed to import ChatGoogleGenerativeAI")):
    ...
    def test_error_handling_with_invalid_api_key(self):
        ...
        with patch("ryoma_ai.llm.provider.load_model_provider", side_effect=Exception("Invalid API key")):
