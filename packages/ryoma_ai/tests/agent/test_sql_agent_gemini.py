...
class TestSQLAgentGeminiIntegration:
    ...
    def test_sql_agent_gemini_initialization(self, mock_load):
        ...
        mock_load.assert_called_once_with("gemini:gemini-2.0-flash", model_parameters=None)
    ...
    def test_sql_agent_gemini_with_parameters(self, mock_load):
        ...
        mock_load.assert_called_once_with("gemini:gemini-1.5-pro", model_parameters=params)
    ...
    def test_sql_agent_gemini_prompt_creation(self, mock_load):
        ...
        mock_datasource = Mock(spec=SQLDatasource)
        mock_datasource.get_table_schema.return_value = "CREATE TABLE users (id INT, name VARCHAR(100))"
    ...
    def test_sql_agent_error_handling_with_gemini(self, mock_load):
        ...
        with pytest.raises(ImportError, match="Failed to import ChatGoogleGenerativeAI"):
            SQLAgent(
                model="gemini:gemini-2.0-flash",
                datasource=mock_datasource
            )
    ...
    def test_sql_generation_with_gemini_mock_response(self, mock_load):
        ...
        mock_datasource = Mock(spec=SQLDatasource)
        mock_datasource.get_table_schema.return_value = "CREATE TABLE products (id INT, name VARCHAR(100), price DECIMAL)"
        ...
        with patch.object(agent, 'chain') as mock_chain:
            mock_response = Mock()
            mock_response.content = "SELECT name, price FROM products WHERE price > 100;"
    ...
    def test_sql_agent_prompt_template_integration(self, mock_load):
        ...
        mock_datasource = Mock(spec=SQLDatasource)
        mock_datasource.get_table_schema.return_value = "CREATE TABLE orders (id INT, customer_id INT, total DECIMAL)"
...
class TestSQLAgentErrorScenarios:
    ...
    def test_model_none_error_message(self, mock_load):
        ...
        with pytest.raises(ValueError, match="Unable to initialize model 'gemini:invalid-model'"):
            agent._build_chain()
