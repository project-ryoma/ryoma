...
class TestSqlApprovalWorkflow:
    ...
    def workflow_agent(self):
        ...
        tools = []
        agent = WorkflowAgent(tools=tools, model="gpt-4")
        return agent
    ...
    def test_approval_response_handling_approve(
        self, enhanced_sql_agent, mock_datasource
    ):
    ...
    def test_approval_response_handling_deny(self, enhanced_sql_agent):
        ...
        with patch(
            ...
            state = {"messages": [], "generated_sql": "SELECT * FROM users LIMIT 10"}
    ...
    def test_approval_response_handling_edit(self, enhanced_sql_agent, mock_datasource):
        ...
        with patch(
            ...
            with patch(
                ...
                messages = result_state.get("messages", [])
                edit_messages = [
                    m for m in messages if "user-edited query" in m.content
                ]
    ...
    def test_workflow_agent_messagestate_handling(self, workflow_agent):
        ...
        mock_workflow.invoke.assert_called_once_with(
            message_state, config=workflow_agent.config
        )
    ...
    def test_stream_command_handling(self, workflow_agent):
        ...
        mock_workflow.stream.assert_called_once_with(
            command, config=workflow_agent.config, stream_mode="values"
        )
    ...
    def full_workflow(self, mock_datasource):
        ...
        with patch("ryoma_ai.agent.internals.enhanced_sql_agent.SqlErrorHandler"):
            with patch(
                ...
                agent = EnhancedSqlAgent(model="gpt-4", datasource=mock_datasource)
                return agent
    ...
    def test_error_handling_during_approval(self, full_workflow, mock_datasource):
        ...
        with patch(
            ...
            with patch(
                ...
                assert (
                    result_state["error_info"]["error_message"]
