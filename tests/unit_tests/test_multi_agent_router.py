...
class MockModel:
    ...
    def invoke(self, prompt: str):
        ...
        if any(word in question_line for word in ['show', 'find', 'select', 'table', 'database']):
            response = '{"task_type": "sql_query", "confidence": 0.9, "reasoning": "Contains SQL/database keywords"}'
        elif any(word in question_line for word in ['write', 'function', 'code', 'script', 'python']):
            response = '{"task_type": "python_code", "confidence": 0.8, "reasoning": "Contains programming keywords"}'
        elif any(word in question_line for word in ['analyze', 'correlation', 'plot', 'trends']):
...
class TestMultiAgentManager(unittest.TestCase):
    ...
    def test_get_capabilities(self):
        ...
        expected_agents = {"SQL Agent", "Python Agent", "Data Analysis Agent", "Chat Agent"}
