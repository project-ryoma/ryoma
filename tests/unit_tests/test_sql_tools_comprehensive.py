...
class TestSqlQueryTool:
    ...
    def test_query_execution_with_datasource_error(self, mock_store_with_datasource):
        ...
        assert isinstance(result, tuple)
        content, artifact = result
        assert "Received an error while executing the query: Database connection failed" in content
        assert artifact == ""
    ...
    def test_query_execution_no_datasource(self):
        ...
        assert isinstance(result, tuple)
        content, artifact = result
        assert "Received an error while executing the query: No datasource available in store" in content
        assert artifact == ""
...
class TestCreateTableTool:
    ...
    def test_successful_table_creation(self, mock_store_with_datasource, sample_columns):
...
class TestQueryProfileTool:
    ...
    def test_query_profile_error(self, mock_store_with_datasource):
        ...
        store, mock_datasource = mock_store_with_datasource
        mock_datasource.get_query_profile.side_effect = Exception("Profile generation failed")
        tool = QueryProfileTool()
...
class TestSchemaAnalysisTool:
    ...
    def test_analyze_specific_table(self, mock_store_with_datasource):
        ...
        result = tool._run(store=store, table_name="test_table", schema_name="test_schema")
...
class TestQueryOptimizationTool:
    ...
    def test_optimization_suggestions(self):
        ...
        result = tool._run(query="SELECT * FROM users WHERE name LIKE '%john'", datasource=mock_datasource)
        assert "Avoid LIKE patterns starting with %" in result
        
        # Test subquery
        result = tool._run(query="SELECT * FROM users WHERE id IN (SELECT user_id FROM orders)", datasource=mock_datasource)
...
class TestEdgeCasesAndFailures:
    ...
    def test_large_query_handling(self, complete_setup):
        ...
        large_query = "SELECT " + ", ".join([f"col_{i}" for i in range(1000)]) + " FROM large_table"
