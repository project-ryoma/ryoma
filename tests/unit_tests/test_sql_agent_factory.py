...
class TestBasicSqlAgent:
    ...
    def test_basic_agent_unsupported_methods(self, mock_datasource):
        ...
        with pytest.raises(NotImplementedError, match="Safety rules require enhanced or reforce mode"):
            agent.enable_safety_rule("no_drop_tables")
        
        with pytest.raises(NotImplementedError, match="Safety rules require enhanced or reforce mode"):
            agent.disable_safety_rule("no_drop_tables")
        
        with pytest.raises(NotImplementedError, match="Safety configuration requires enhanced or reforce mode"):
            agent.set_safety_config({"enable_validation": True})
        
        with pytest.raises(NotImplementedError, match="Schema analysis requires enhanced or reforce mode"):
            agent.analyze_schema("Find customer data")
        
        with pytest.raises(NotImplementedError, match="Query planning requires enhanced or reforce mode"):
...
class TestEnhancedSqlAgentImpl:
    ...
    def test_enhanced_agent_initialization(self, mock_enhanced_class, mock_datasource):
        ...
        expected_tools = [
            ...
            "schema_analysis", "query_validation", "query_optimization", "query_explanation"
        ]
        for tool_name in expected_tools:
            assert tool_name in tool_names
    ...
    @patch('ryoma_ai.agent.sql.InternalEnhancedSqlAgent')
    def test_enhanced_agent_schema_and_planning_methods(self, mock_enhanced_class, mock_datasource):
        ...
        mock_schema_agent.analyze_schema_relationships.return_value = "Schema analysis result"
        result = agent.analyze_schema("Find customer relationships")
        assert result == "Schema analysis result"
        mock_schema_agent.analyze_schema_relationships.assert_called_once_with("Find customer relationships")
        ...
        mock_query_planner.create_query_plan.assert_called_once_with("Get customer orders", context)
    
    @patch('ryoma_ai.agent.sql.InternalEnhancedSqlAgent')
    def test_enhanced_agent_missing_components(self, mock_enhanced_class, mock_datasource):
...
class TestBackwardCompatibility:
    ...
    def test_can_import_from_agent_module(self):
        ...
        from ryoma_ai.agent import SqlAgent, BasicSqlAgent
        assert SqlAgent is not None
