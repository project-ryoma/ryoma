    ...
    def index_feature_from_source(
        ...
        if feature_view["source_type"] == "PushSource":
            ...
            if push_source_type in ["parquet", "csv", "json", "txt"]:
                ingest_inputs = self.process_file_source(
                    feature_view, feature_source_path, push_source_type
                )
            elif push_source_type == "pdf":
                ingest_inputs = self.process_pdf_source(
            ...
            if embedding_client:
                ingest_inputs = self.apply_embedding(embedding_client, ingest_inputs)
            self.index_feature_from_data(fs, feature_view, ingest_inputs)
        else:
